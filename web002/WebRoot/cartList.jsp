							...
							<tbody>
								<tr>
									<td align="center">
										...
										<input type="button" value="车源信息" onclick="location='/makehtml/servlet/CartListServlet?p=1';" class="coolbg np" style="color: #3996f4;">
				...
				<tbody>
					...
					<c:forEach items="${cartList}" varStatus="status" var="cart">
						<tr bgcolor="#FFFFFF" align="center" height="22"
							...
							<td>${status.count}</td>
							<td><input type="checkbox" class="np" value=${cart.id} id="id" name="id"></td>
							<td align="left"><a title="${cart.title}">${fn:substring(cart.title, 0, 26)}</a></td>
							<td>${cart.link_man}</td>
							<td>${cart.link_tel}</td>
							<td>${cart.start_address}</td>
							<td>${cart.dest_address}</td>
							<td>${cart.modify_time}</td>
							<td><a href="#" onclick="addCartAajax(${cart.id})">添加</a></td>
						</tr>
					</c:forEach>
					<tr bgcolor="#FAFAF1">
						<td height="28" colspan="10">
							...
							<a class="coolbg" href="javascript:batchCart(0,'add')">&nbsp;添加&nbsp;</a>
							<a class="coolbg" href="javascript:batchCart(0,'del')">&nbsp;删除&nbsp;</a>
						</td>
					</tr>
					<tr bgcolor="#EEF4EA" align="right">
						<td align="center" height="36" colspan="10">
							<c:if test="${p!=1}">
								<a href="/makehtml/servlet/CartListServlet?p=1">首页</a>&nbsp;
								<a href="/makehtml/servlet/CartListServlet?p=${p-1}">上页</a>&nbsp;
							</c:if>
							<c:if test="${p==1}">首页&nbsp;上页&nbsp;</c:if>
							<c:if test="${p==lastPage}">下页&nbsp;尾页</c:if>
							<c:if test="${p!=lastPage}">
								<a href="/makehtml/servlet/CartListServlet?p=${p+1}">下页</a>&nbsp;
								<a href="/makehtml/servlet/CartListServlet?p=${lastPage}">尾页</a>
							</c:if>
						</td>
					</tr>
				</tbody>
